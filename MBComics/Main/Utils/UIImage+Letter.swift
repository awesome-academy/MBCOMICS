//
//  UIImage+Letter.swift
//  MBComics
//
//  Created by HoaPQ on 6/9/20.
...
extension UIImageView {
    ...
    private func imageSnap(text: String?,
        ...
        let attributes = textAttributes ?? [.foregroundColor: UIColor.white,
                                            .font: UIFont.systemFont(ofSize: 55.0)]
    
        //stroke color
        if stroke {
            
            //outer circle
            context?.setStrokeColor((attributes[.foregroundColor] as? UIColor ?? .white).cgColor)
            context?.setLineWidth(4)
            var rectangle: CGRect = CGRect(x: 0,
                                           y: 0,
                                           width: size.width,
                                           height: size.height)
